...
void pid_controller(void)
{
	...
	if(pid.currentPosition > 1400 & motor.direction == UP)
		{ pid.currentPosition = 1; }
	...
	if(pid.distance < -120)
		{ motor.speed = 190; motor.direction = UP; }
	else if (pid.distance > 100)
		{ motor.speed = 190; motor.direction = DOWN; }
	else{
		if(pid.distance > 0)
...
/*
	...
	pid.distance = (pid.kp*pid.error) + (pid.ki*pid.integral) + (pid.kd*pid.derivative);
	printf("distance: %d\n\n\r",pid.distance);
	
	if(pid.distance > maxDistance)
	pid.distance = maxDistance;
	else if (pid.distance < minDistance)
	pid.distance = minDistance;
	
	
	
	if (pid.distance > 255)
	{ motor.speed = 65; motor.direction = DOWN; }
	else if (pid.distance < -255)
	{ motor.speed = 65; motor.direction = UP; }
	
	if (pid.distance > 0)
	{ motor.direction = DOWN; motor.speed = pid.distance; }
	else if (pid.distance < 0)
	{ motor.direction = UP; motor.speed = pid.distance; }
	else
	motor.speed = 0;
