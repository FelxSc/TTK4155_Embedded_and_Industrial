...
void setFirstChild(menu_t *parent, menu_t* firstChild)
{
	parent->firstChild = firstChild;
}

// Assign the next menu option in the list
void setRightSibling(menu_t* thisChild, menu_t* rightSibling)
{
...
void menuInit( void )
{
	...
	menu_t* got = createMenu("GameOfThrones", music, GOT);
	...
	switch(joystick_data.joystickPosition)
	{
		...
		case RIGHT:
			if (currentMenu->firstChild != NULL)
			{
				...
				currentLine = 1; // set cursor to first line in new menu window
				drawMenu(currentMenu);
			}				
			break;
		default: break; // If Joystick direction : CENTER - Do nothing
