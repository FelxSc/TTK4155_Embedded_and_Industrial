...
CAN_message_t handleCANInterrupt()
{
	...
	receiveCANmesssage(&receivedCAN);
			...
			state = MCP2515_readRXstatus();
			printf("RX Status = %x\n\r", state);
			
			state = MCP2515_readRxTxStatus();
			printf("RX TX Status = %x\n\r", state);
...
// Return received message to test connection between nodes
 void returnCANmsg( CAN_message_t data )
 {
		...
		printf("Game_data.ID = %d\n\r", data.ID);
		printf("Game_data.msg[0] = %d\n\r", data.msg[0]);
		printf("Game_data.msg[1] = %d\n\r", data.msg[1]);
		printf("Game_data.msg[2] = %d\n\r", data.msg[2]);
		printf("Game_data.msg[3] = %d\n\r", data.msg[3]);
		printf("Game_data.msg[4] = %d\n\r", data.msg[4]);
